...
def yesno(req: SearchWorker):
    ...
    worker_name = model.invoke(
        [
            (
                "user",
                f"Your role is to return if we should answer yes or no based on the information, just answer loosely if it's possible. Return a JSON formatted as {{'search_result': ['yes with explanations']}}, or {{'search_result': ['no with explanations']}}",
                f"Here is the information : {req.knowledge}",
            ),
            ("user", f"Here is the user input : {req.input_text}"),
        ],
